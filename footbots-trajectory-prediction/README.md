...
- 本実装は研究プロトタイプであり、審判、選手評価、賭博、医療など高リスク用途を目的としていません。

## 再現性と公開時の注意

- 動画の著作権・肖像権・利用規約を確認してください。
- 学習済み重みを公開する場合は、学習データの再配布条件を確認してください。
- 個人のローカルパスや未公開データをコミットしないでください。
- `python scripts/verify_repo.py`で公開前チェックを実行できます。
